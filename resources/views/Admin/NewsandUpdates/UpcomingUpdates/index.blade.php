...
@section('content')
    <div class="container">
        <form method="POST" action="{{route('admin.newsandupdates.upcomingupdates.add')}}">
            @csrf
                ...
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        @if(isset($upcomingupdates->title))
                            <input type="text" hidden="" name="id" value="{{$upcomingupdates->id}}"class="form-control">
                            <input type="text" name="title" value="{{$upcomingupdates->title}}" class="form-control" placeholder="Title">
                        @else
                     ...
                     <div class="form-group">
                        <label>Description</label>
                         @if(isset($upcomingupdates->description))
                           <textarea id="myTextarea" name="description" value="{{$upcomingupdates->description}}" style="width: 100%;">{{$upcomingupdates->description}}</textarea>
                        @else
